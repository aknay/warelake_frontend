...
String generateRandomPassword({int length = 12}) {
  ...
  List<String> result =
      List.generate(length, (index) => charset[random.nextInt(charset.length)]);
...
DateTime generateRandomExpiredDate() {
  ...
  // Calculate a random number of seconds between last week and next year
  Random random = Random();
  int randomSeconds = random.nextInt(nextYear.difference(lastWeek).inSeconds);

  // Create a new DateTime by adding the random number of seconds to lastWeek
  DateTime randomDateTime = lastWeek.add(Duration(seconds: randomSeconds));
...
CreateItemRequest getShirtItemRequest(
  ...
  final whiteShirt = getWhiteShirt(
      salePriceInInt: salePriceInInt,
      purchasePriceInInt: purchasePriceInInt,
  ...
  final blackShirt = getBlackShirt(
      ...
      purchasePriceInInt: purchasePriceInInt,
      expiryDate: expiryDate,
      name: secondShirtName);
  return CreateItemRequest(
      item: Item.create(name: "shirt", unit: 'pcs'),
...
CreateItemRequest getJeanItemRequest(
    {int? salePriceInInt, int? purchasePriceInInt}) {
  final whiteJean = getWhiteJean(
      salePriceInInt: salePriceInInt, purchasePriceInInt: purchasePriceInInt);
...
List<LineItem> getLineItemsWithRandomCount(
    {required List<ItemVariation> items}) {
  return items
      .map((e) => LineItem.create(
          itemVariation: e,
          rate: e.purchasePriceMoney.amountInDouble,
          quantity: Random().nextInt(5) + 5,
          unit: 'pcs'))
      .toList();
}

List<LineItem> getLineItems(
    {required List<Tuple2<double, List<ItemVariation>>> items}) {
  return items
...
List<LineItem> getLineItemIndividual(
    {required List<Tuple2<double, ItemVariation>> items}) {
  return items
      .map((e) => LineItem.create(
          itemVariation: e.value2,
          rate: e.value2.purchasePriceMoney.amountInDouble,
          quantity: e.value1,
          unit: 'pcs'))
      .toList();
}

List<StockLineItem> getStocklLineItemWithRandomCount(
    {required List<ItemVariation> createdItemList}) {
  return createdItemList
      .map((e) => StockLineItem.create(
          itemVariation: e, quantity: Random().nextInt(5) + 5.0))
      .toList();
}

List<StockLineItem> getStockLineItem(
    {required List<Tuple2<double, List<ItemVariation>>> items}) {
  return items
      .map((e) => e.value2.map(
          (f) => StockLineItem.create(itemVariation: f, quantity: e.value1)))
      .flattened
      .toList();
}

List<StockLineItem> getStockLineItemWithIndividual(
    {required List<Tuple2<double, ItemVariation>> items}) {
  return items
      .map((e) =>
          StockLineItem.create(itemVariation: e.value2, quantity: e.value1))
      .toList();
}
