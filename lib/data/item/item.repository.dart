...
class ItemRepository extends ItemApi {
    ...
    try {
      ...
      if (itemSearchField != null) {
        if (itemSearchField.startingAfterItemId != null) {
          additionalQuery["starting_after"] =
              itemSearchField.startingAfterItemId!;
        }
      ...
      if (response.statusCode == 200) {
        final listResponse =
            ListResponse.fromJson(jsonDecode(response.body), Item.fromJson);
  ...
  Future<Either<ErrorResponse, Item>> getItem(
    ...
    try {
      final response = await HttpHelper.get(
          url: ApiEndPoint.getItemEndPoint(itemId: itemId),
          token: token,
          teamId: teamId);
  ...
  Future<Either<ErrorResponse, Item>> createItemRequest(
    ...
    try {
      final response = await HttpHelper.post(
          ...
          token: token,
          teamId: teamId);
      log("here while creating an item: response ${jsonDecode(response.body)}");
      ...
      return Left(ErrorResponse.fromJson(
          json: jsonDecode(response.body), statusCode: response.statusCode));
    } catch (e) {
      log("the error is $e");
      return Left(ErrorResponse.withOtherError(message: e.toString()));
    }
  ...
  //     {required String itemId, required String itemVariationId, required String teamId, required String token}) async {
  //   try {
  //     final response = await HttpHelper.get(
  //         url: ApiEndPoint.getItemVariationEndPoint(itemId: itemId, itemVariationId: itemVariationId),
  //         token: token,
