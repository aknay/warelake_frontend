...
@riverpod
class AsyncItemVariationListByItemIdController extends _$AsyncItemVariationListByItemIdController {
  @override
  Future<List<ItemVariation>> build({required String itemId}) {
    return _getItemVariations(itemId: itemId);
  }

  Future<List<ItemVariation>> _getItemVariations({required String itemId}) async {
    if (kDebugMode) {
      await Future.delayed(const Duration(seconds: 1));
    }
    final itemOrError = await ref.read(itemServiceProvider).getItemVariations(itemId: itemId);
